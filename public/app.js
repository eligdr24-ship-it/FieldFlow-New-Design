...
function renderOpportunities(){
 ...
 ].map(g=>`<article class="gigCard"><div class="gigImg"><h3>${g[0]}</h3></div><div class="gigBody"><div class="rating">★★★★★ 4.9</div><p><b>${g[1]}</b><br>${g[2]}</p><div class="priceLine"><span>${g[4]}</span><b>${g[3]}</b></div><button class="primary" onclick="toast('Opportunity opened')">View Details</button></div></article>`).join('');
}
function renderPros(){
 $('#proList').innerHTML=techs.map(t=>`<div class="proItem"><div class="avatar">${t[1].split(' ').map(x=>x[0]).join('')}</div><div><b>${t[0]}</b><small>${t[1]} • ${t[2]} • ★ ${t[3]} • ${t[4]}</small></div><button class="ghost" onclick="switchView('profile')">View</button></div>`).join('');
}
function renderActivity(){
 $('#activityFeed').innerHTML=['Mike accepted a lead in Philadelphia|2 min ago','Invoice uploaded for Plano repair|11 min ago','Publisher posted emergency spring job|18 min ago','Tom marked arrived on site|32 min ago','Payment released to Chris Davis|1 hour ago'].map(x=>{const a=x.split('|');return `<div class="activityItem"><span class="status paid">●</span><div><b>${a[0]}</b><small>${a[1]}</small></div></div>`}).join('');
}
const trustData={
...
function renderTechCards(){
 $('#techCards').innerHTML=techs.map(t=>`<article class="techCard"><h3>${t[0]}</h3><p class="muted">${t[1]} • ${t[2]}</p><div class="badgeRow"><span>★ ${t[3]}</span><span>${t[4]}</span><span>${t[5]}</span><span>${t[6]}</span></div><p><b>Categories:</b> ${t[7]}<br><b>ZIPs:</b> 19116, 19115, 19020, 19053</p><button class="primary" onclick="toast('Partial lead sent to ${t[0]}')">Send Lead</button> <button class="ghost" onclick="switchView('profile')">View Profile</button></article>`).join('');
}
